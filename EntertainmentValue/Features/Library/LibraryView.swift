...
struct LibraryView: View {
    ...
    .toolbar {
      ...
      ToolbarItem(placement: .topBarTrailing) {
        Menu("Add", systemImage: "plus") {
          Button("Add Title", systemImage: "books.vertical") {
            navigation.presentedSheet = .quickAdd()
          }
          Button("Add & Log", systemImage: "checkmark") {
            navigation.presentedSheet = .quickAdd(logsAfterAdding: true)
          }
        }
      }
  ...
  private var emptyTitle: LocalizedStringKey {
    ...
    case .kind(let kind): "No \(kind.displayName) yet"
    }
  }
