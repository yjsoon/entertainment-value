  ...
  var body: some View {
    Form {
      ...
      Section {
        ...
        Text("Log uses this period to show what you have already enjoyed. The default is Month.")
      }
      ...
      Section {
        ...
        Text(
          "Keys are stored in your device's Keychain, take effect immediately, and are never included in exports or backups."
        )
      }

      Section("About") {
        Link(
          "Entertainment Value on GitHub",
          destination: URL(string: "https://github.com/yjsoon/entertainment-value")!)
    ...
    Button {
      ...
      LabeledContent {
        Label(statusText, systemImage: statusSymbol)
          .foregroundStyle(
            status.isUsable ? EntertainmentValueTheme.sage : EntertainmentValueTheme.coral)
      } label: {
  ...
  var body: some View {
    NavigationStack {
      Form {
        ...
        Section {
          SecureField(
            status.maskedKey == nil ? "Paste key" : "Paste replacement key", text: $draftKey
          )
        ...
        if let maskedKey = status.maskedKey {
          Section {
            ...
            if hasConfigFallback {
              ...
              Text(
                "Removing the saved key disables \(credentialKey.displayName) search until you add another."
              )
            }
          }
        }
