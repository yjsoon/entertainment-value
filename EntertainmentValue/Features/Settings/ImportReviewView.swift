...
private struct ImportReviewRow: View {
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            ForEach(row.warnings) { warning in
                Label(warning.message, systemImage: warning.severity == .warning ? "exclamationmark.triangle" : "info.circle")
                    .font(.footnote)
                    .foregroundStyle(
                        warning.severity == .warning ? Color.orange : EntertainmentValueTheme.secondaryInk
                    )
            }
        }
        .padding(.vertical, 5)
    ...
    private var confidenceColor: Color {
        if row.confidence >= 0.85 { return EntertainmentValueTheme.sage }
        if row.confidence >= 0.6 { return .orange }
        return EntertainmentValueTheme.coral
    }
