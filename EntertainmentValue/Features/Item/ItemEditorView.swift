...
struct ItemEditorView: View {
  ...
  private let onDidSave: ((LibraryItem) -> Void)?
  ...
  init(
    ...
    onDidSave: ((LibraryItem) -> Void)? = nil
  ) {
    self.itemID = itemID
  ...
  private var typeSpecificSection: some View {
    ...
    case .tvShow:
      Section("TV Show") {
        Text(
          "Seasons and episodes can be added from the item detail. Season ratings derive the show rating until you override it here."
        )
        .font(.footnote)
        .foregroundStyle(EntertainmentValueTheme.secondaryInk)
      }
  ...
  private var reminderSection: some View {
    Section("Start Date") {
      ...
      if draft.hasStartReminder {
        ...
        Text(
          "If it becomes overdue, Entertainment Value leaves it quietly in your library without repeated nags."
        )
        .font(.footnote)
        .foregroundStyle(EntertainmentValueTheme.secondaryInk)
      }
...
extension String {
  ...
  fileprivate var commaSeparated: [String] {
    split(separator: ",")
