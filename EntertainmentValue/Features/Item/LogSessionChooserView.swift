...
struct LogSessionChooserView: View {
  ...
  var body: some View {
    NavigationStack {
      List {
        ...
        Section {
          Button("Add a New Title", systemImage: "plus") {
            onAddNew()
          }
        } footer: {
          Text("Can’t find it here? Add it to your library, then log it.")
        }
      }
  ...
  private func itemRows(_ items: [LibraryItem]) -> some View {
    ForEach(items) { item in
      ...
      .accessibilityLabel(
        "Log a session for \(item.title), \(String(localized: item.mediaKind.singularName))")
    }
  }
...
#Preview("Log Session Chooser") {
  ...
  LogSessionChooserView(onSelect: { _ in }, onAddNew: {})
