...
struct ItemDetailView: View {
    ...
    private func toolbar(for item: LibraryItem) -> some ToolbarContent {
        ToolbarItem(placement: .topBarTrailing) {
            ...
            .tint(item.isFavorite ? EntertainmentValueTheme.coral : nil)
        }
    ...
    var body: some View {
        VStack(spacing: 18) {
            CoverArtworkView(item: item, contentMode: .fit)
                ...
                .shadow(color: EntertainmentValueTheme.ink.opacity(0.18), radius: 16, y: 9)
                        ...
                        HStack(spacing: 8) {
                            ForEach(facets) { facet in
                                Text(facet.name)
                                    ...
                                    .padding(.vertical, 6)
                                    .background(facet.kind == .tag ? EntertainmentValueTheme.coral.opacity(0.16) : EntertainmentValueTheme.sky.opacity(0.14), in: .capsule)
                            }
                        }
                    }
                ...
                ForEach(events.prefix(12)) { event in
                    HStack(spacing: 10) {
                        Image(systemName: symbol(for: event.kind))
                            .foregroundStyle(event.kind == .completed ? EntertainmentValueTheme.sage : EntertainmentValueTheme.coral)
                        VStack(alignment: .leading, spacing: 2) {
