...
enum BuiltInListKind: String, CaseIterable, Identifiable, Sendable {
    ...
    var color: Color {
        switch self {
        case .favorites: EntertainmentValueTheme.coral
...
struct ListsView: View {
        ...
        List {
            Section("Quick Views") {
                ForEach(BuiltInListKind.allCases) { kind in
                    ...
                    .listRowBackground(EntertainmentValueTheme.raisedBackground)
                }
            }
    ...
    private func listLink(_ list: UserList) -> some View {
        ...
        .swipeActions(edge: .trailing) {
            ...
            .tint(EntertainmentValueTheme.sky)
        }
        .swipeActions(edge: .leading) {
            ...
            .tint(EntertainmentValueTheme.sage)
        }
        .contextMenu {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Image(systemName: list.kind == .smart ? "wand.and.stars" : "rectangle.stack")
                .font(.title3)
                .foregroundStyle(list.kind == .smart ? EntertainmentValueTheme.sky : EntertainmentValueTheme.coral)
                .frame(width: 28)
