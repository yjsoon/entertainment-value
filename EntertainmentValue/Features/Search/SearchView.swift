...
struct SearchView: View {
  ...
  private let onDidAddItem: ((LibraryItem) -> Void)?
  ...
  init(
    ...
    onDidAddItem: ((LibraryItem) -> Void)? = nil,
    ...
    self.onDidAddItem = onDidAddItem
  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      Divider()
        .overlay(EntertainmentValueTheme.secondaryInk.opacity(0.15))
      ...
      case .loaded(let results):
        if results.isEmpty {
          SearchMessageRow(
            ...
            message:
              "Try another title or add this \(String(localized: selectedMediaKind.singularName)) manually."
          )
          manualAddRow
        } else {
          ForEach(results) { result in
  ...
  private var remoteAttribution: MetadataAttribution? {
    if case .loaded(let results) = remoteState,
...
private struct RemoteSearchResultRow: View {
  ...
  private var metadataSubtitle: String? {
    let creator =
      if result.mediaType == .book || result.mediaType == .comic {
...
private struct SearchMetadataArtwork: View {
  ...
  var body: some View {
    ZStack {
      LinearGradient(
        colors: [mediaKind.accentColor.opacity(0.68), EntertainmentValueTheme.raisedBackground],
      ...
      Image(systemName: didFail ? "wifi.slash" : mediaKind.symbolName)
        ...
        .foregroundStyle(EntertainmentValueTheme.ink.opacity(0.7))
...
struct QuickAddView: View {
  ...
  private let logsAfterAdding: Bool
  ...
  init(
    ...
    self.logsAfterAdding = logsAfterAdding
  ...
  private func selectLocalItem(_ item: LibraryItem) throws -> Bool {
    ...
    if logsAfterAdding {
      navigation.chooseItemToLog(item.id)
      return false
    }
  ...
  private func finishAdding(_ item: LibraryItem) {
    guard logsAfterAdding else { return }
    navigation.logAfterCurrentSheetDismisses(item.id)
    dismiss()
  }
