...
struct RootView: View {
  ...
  var body: some View {
    ...
    TabView(selection: $navigation.selectedTab) {
      Tab("Log", systemImage: "clock", value: .home) {
    ...
    .sheet(
      item: $navigation.presentedSheet,
      onDismiss: {
      ...
      case .quickAdd(let initialMediaKind, let destinationListID, let logsAfterAdding):
        QuickAddView(
          ...
          logsAfterAdding: logsAfterAdding
        )
        .environment(navigation)
      case .addItem:
        ItemEditorView()
      ...
      case .chooseItemToLog:
        LogSessionChooserView(
          onSelect: navigation.chooseItemToLog,
          onAddNew: navigation.addNewTitleToLog
        )
  ...
  private func ensureDailySnapshot() async -> Bool {
    ...
    do {
      ...
      let gridStyle =
        defaults.string(forKey: "library.grid-style")
